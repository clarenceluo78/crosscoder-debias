...
class CrossCoder(nn.Module):
    def __init__(self, cfg):
        ...
        if self.cfg.get("layer_length", None) is not None:
        ...
        self.W_enc = nn.Parameter(
            torch.empty(self.layer_len, d_in, d_hidden, dtype=self.dtype)
        )
        self.W_dec = nn.Parameter(
            torch.nn.init.normal_(
                torch.empty(
                    d_hidden, self.layer_len, d_in, dtype=self.dtype
                )
            )
        )
        self.W_dec = nn.Parameter(
        ...
        self.b_dec = nn.Parameter(
            torch.zeros((self.layer_len, d_in), dtype=self.dtype)
        )
    ...
    @classmethod
    def load(cls, version_dir, checkpoint_version, save_dir="/data2/hluo/checkpoints/crosscoders"):
        save_dir = Path(save_dir) / str(version_dir)
