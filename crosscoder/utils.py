...
def load_pile_lmsys_mixed_tokens():
    try:
        ...
        all_tokens = torch.load("/data2/hluo/dataset/pile-lmsys-mix-1m-tokenized-gemma-2.pt")
    except:
        print("Data is not cached. Loading data from HF")
        data = load_dataset(
            "ckkissane/pile-lmsys-mix-1m-tokenized-gemma-2", 
            split="train", 
            cache_dir="/data2/hluo/cache/"
        )
        data.save_to_disk("/data2/hluo/dataset/pile-lmsys-mix-1m-tokenized-gemma-2.hf")
        data.set_format(type="torch", columns=["input_ids"])
        all_tokens = data["input_ids"]
        torch.save(all_tokens, "/data2/hluo/dataset/pile-lmsys-mix-1m-tokenized-gemma-2.pt")
        print(f"Saved tokens to disk")
    return all_tokens


def compile_all_tokens(
        sequence_length=256, 
        batch_size=512, 
        max_batches=1000, 
        device='cuda:0', 
